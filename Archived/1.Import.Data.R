...
dat.length.11<-gs_title("Lobsters_data_2018_All")%>% # To use GoogleSheets
  gs_read_csv(ws = "Lobster.var" )%>%
  glimpse()


dat.length.1<-dat.length.11%>%
  mutate(Count=1)%>%        #Count for Abundance
  #filter(!is.na(Carapace.length))%>% #Turn on if you want to filter out NA CL
  filter(!is.na(Tag.number))%>% #Turn on if you want to filter out NA Tag.numbers
...
# filter(dat.length.1,Total.damage==5)%>%
#   glimpse()
...
dat.1 <- dplyr::left_join(dat.length.1, dat.pot.1)%>%
...
#Add a "Site" column by Trap.ID to dat.length----
dat.length.2<-left_join(dat.length.2,sites.2, by="Trap.number") 
...
dat.2 <- dplyr::left_join(dat.length.2, dat.pot.2)%>%
...
length(unique(dat.2$Tag.number))
#1683
