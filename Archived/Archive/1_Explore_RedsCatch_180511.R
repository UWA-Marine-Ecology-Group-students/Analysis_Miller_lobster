...
# work.dir=("~/Google Drive/Project_WRL low-catch zone/Fieldwork and Reporting/02_Diving/Analysis_WRL_Dive_Surveys") #For Oscar
# work.dir=("C:/Users/21301367/Google Drive/Project_WRL low-catch zone/Fieldwork and Reporting/02_Diving/Analysis_WRL_Dive_Surveys") #For Brooke
# work.dir=("C:/Users/20934517/Google Drive/Project_WRL low-catch zone/Fieldwork and Reporting/02_Diving/Analysis_WRL_Dive_Surveys") #For Michael
...
dat.length<-dat.length.gsheet%>%
  ...
  mutate(Trap.Number.2=as.numeric(str_extract(ID, "[0-9]+")))%>%
  mutate(Site=(str_extract(ID, "[aA-zZ]+")))%>%
...
# without filtering out unknown 4277 with filters = 4116
length(unique(dat.length$day.site.trap)) # 319
...
#   # dplyr::rename(Location=Group.site)%>% #TJL changed here to make consistent! 
#   # mutate(Location=str_replace_all(.$Location,c("SM"="Seven Mile","CH"="Cliff Head","WP"="Whitepoint")))%>%
#   mutate(Location=fct_relevel(Location,"Seven Mile Beach", "Whitepoint"))%>%
...
length(unique(dat.pot$day.trap)) # 450 - including pots with no lobster
...
dat.factors<-dat.length%>%
  select(ID,Trap.Number,Day,Site,Date)%>% # changed location to Location BG
  distinct()%>%
  glimpse()
# 
# # # Make histogram data----
# dat.hist<-dat.length%>%
#   left_join(dat.factors,by=c("ID"))%>%
# #   filter(!is.na(Location))%>% #Remove this once the 4 missing pots are fixed
#   glimpse()
...
dat.all<-dat.length%>%
  ...
  summarize(n())%>%
  rename(Abundance="n()")%>%
  right_join(dat.factors,by=c("ID"))%>%
