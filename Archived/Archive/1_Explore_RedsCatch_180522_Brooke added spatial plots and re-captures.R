...
bar.total.prop<-ggplot(data = dat.prop, aes(x = Site.Name,y=total.prop)) + 
  stat_summary(fun.y=mean, geom="bar",fill="white",colour="black") +
  stat_summary(fun.ymin = se.min, fun.ymax = se.max, geom = "errorbar", width = 0.1) +
  geom_hline(aes(yintercept=0))+
  xlab("")+
  ylab("Total proportion recaptured")+
  theme_bw()+Theme1+ 
  theme(axis.line.x=element_line(colour="white", size=0.5,linetype='solid'),
        axis.ticks.x=element_line(colour="white", size=0.5,linetype='solid'))
bar.total.prop
ggsave(bar.total.prop,file=paste(study,"bar.total.prop-draft-AM", "png",sep = "."), width = 15, height = 15,units = "cm")
