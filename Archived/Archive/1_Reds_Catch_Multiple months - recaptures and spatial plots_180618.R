...
unique(dat.length$Trap.ID) # 359 traps

# # Import pot data----
dat.pot <-gs_title("Lobsters_data_2018_Reds_20180511")%>%
   gs_read_csv(ws = "Pot.var")%>%
   mutate(day.trap=paste(Day,Pot.Number,sep="."))%>%
   mutate(Site.Name=str_replace_all(.$Site.Name,c("SM"= "Seven Mile", "RM"="Rivermouth", "DM"="Davidsmarks", "LR"="Long Reef", "SD"="South Dummy",  "IR"="Irwin Reef","LH"="Little Horseshoe", "CHin1_"="Cliff Head IN1","CHin2_"="Cliff Head IN2","CHout1_" = "Cliff Head OUT1","CHout2_" = "Cliff Head OUT2", "JB"="Jim Bailey", "GR"="Golden Ridge", "SR"="South Rig", "WL"="Whites Lump")))%>% 
   filter(Johns=="No")%>% # turn off if you add in john's data
   glimpse()

write.csv(dat.pot,"dat.pot.csv")
dat.pot<-read.csv("dat.pot.csv")
unique(dat.pot$Trap.ID) # 372 pots
...
legal.map.legal<-gplot(map, maxpixels = 100000000000000) + #, maxpixels = 5e101000000000000000
  geom_tile(aes(fill = value)) +
  scale_fill_gradientn(colours=used_cols, guide=FALSE)+
  coord_equal()+
  Theme_spatial+
  ...
  geom_point(data=filter(dat.map,Abundance<1&Legal=="Legal"),aes(lon,lat),colour="black",fill="red",size=2,alpha=0.3,shape=4)+
  scale_size_continuous(range = c(3,5), breaks=seq(0,12.5, by=2.5), name = "Abundance per pot")+
...
legal.map.sublegal<-gplot(map, maxpixels =100000000000000) + #, maxpixels = 5e10
  geom_tile(aes(fill = value)) +
  #facet_wrap(~ variable) +
  ...
  geom_point(data=filter(dat.map,Abundance>0),aes(lon,lat,size=Abundance),shape=21,colour="black",fill="dodgerblue",alpha=0.3)+
  geom_point(data=filter(dat.map,Abundance<1&Legal=="Legal"),aes(lon,lat),colour="black",fill="red",size=2,alpha=0.3,shape=4)+
  scale_size_continuous(range = c(3,10), breaks=seq(0,75, by=25), name = "Abundance per pot")+
  scale_shape_manual(values=c(21,24), name = "")+
  Theme_spatial+
  xlab('Longitude')+
  ylab('Latitude')+
...
setwd(data.dir)
write.csv(table,"Numbers.released.and.recaptured.May.June.csv")
