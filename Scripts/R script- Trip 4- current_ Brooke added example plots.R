...
# Import length data----
dat.length.6 <- read.csv("Lobsters.var.trip6.csv")%>% # For Ash
  glimpse()
...
write.csv(dat.length.6,"lob.dat.csv")
dat.length.6<-read.csv("lob.dat.csv")

unique(dat.length.6$Trap.ID) # 110 levels


# # Import pot data----
dat.pot.6 <-read.csv("Pot.var.trip6.csv")%>% # For Ash
...
dat.pot.6$Site.Name
write.csv(dat.pot.6,"dat.pot.csv")
dat.pot.6<-read.csv("dat.pot.csv")
unique(dat.pot.6$Trap.ID) # 98 pots
...
dat.length.6<-left_join(dat.length.6,sites.6)



# Checks---
unique(dat.pot.6$day.trap)
length(unique(dat.pot.6$day.trap)) # 170

# # Make Hist data and Summarise the length data to abundance for the all/legal/sublegal----
# Make factors----
dat.factors.6<-dat.pot.6%>%
...
unique(dat.factors.6$Trap.ID) # 98 pots

dat.factors.6$Site.Name

# # # Make histogram data----
dat.hist.6<-dat.length.6%>%
  left_join(dat.factors.6,by=c("Trap.ID"))%>%
  #filter(!is.na(Site))%>% # Didn't work, not sure if we need this line anyway-AM.
  glimpse()

check<-dat.length.6%>% # this will be any that in lengths but not in pot.var
  dplyr::select(Trap.ID,Day)%>%
  anti_join(.,dat.pot.6)


# Make abundance of legal/sublegal----
dat.sublegal.6<-dat.length.6%>%
  left_join(dat.factors.6)%>%
  filter(Carapace.length<76)%>%
  group_by(Trap.ID,Date.Recovered)%>%
  summarize(Abundance=n())%>%
  right_join(dat.factors.6,by = c("Trap.ID", "Date.Recovered"))%>%
  replace_na(list(Abundance = 0))%>%
  mutate(Legal="Sublegal")%>%
  glimpse()

dat.legal.6<-dat.length.6%>%
  left_join(dat.factors.6)%>%
  filter(Carapace.length>76)%>%
  group_by(Trap.ID,Date.Recovered)%>%
  summarize(Abundance=n())%>%
...
dat.all.6<- bind_rows(dat.sublegal.6,dat.legal.6)

dat.all.6$Site.Name

# Check--
unique(dat.all.6$Legal)
...
dat.all.6$Site.Name<-factor(dat.all.6$Site.Name, levels = c("Irwin Reef", "Whites Lump", "Jim Bailey","South Rig", "Little Horseshoe","Cliff Head OUT2","Cliff Head IN1", "Cliff Head IN2", "Golden Ridge"))
...
summary(dat.all.6$Abundance)
dat.all.6$Site.Name 

#Jitter Plot

bar.location.all.jitter.6<-ggplot(data = dat.all.6, aes(x = Site.Name,y=Abundance)) + 
  stat_summary(fun.y=mean, geom="bar",fill="white",colour="black") +
  stat_summary(fun.ymin = se.min, fun.ymax = se.max, geom = "errorbar", width = 0.1) +
  geom_jitter(width = 0.25, height = 0,alpha=0.25)+
  geom_hline(aes(yintercept=0))+
  ggtitle ("Trip 6 (November)")+
  xlab("")+
...
bar.location.all.jitter.6 



#Abundance plot
bar.location.all.6<-ggplot(data = dat.all.6, aes(x = Site.Name,y=Abundance)) + 
  stat_summary(fun.y=mean, geom="bar",fill="white",colour="black") +
  stat_summary(fun.ymin = se.min, fun.ymax = se.max, geom = "errorbar", width = 0.1) +
  geom_hline(aes(yintercept=0))+
  ggtitle ("Trip 6 (November)")+
...
dat.legal.6$Site.Name<-factor(dat.legal.6$Site.Name, levels = c("Irwin Reef","Whites Lump", "Jim Bailey", "South Rig", "Little Horseshoe","Cliff Head OUT2","Cliff Head IN1", "Cliff Head IN2", "Golden Ridge"))
glimpse(dat.legal.6)
dat.legal.6$Site.Name

# Rename sites

bar.location.legal.6<-ggplot(data = dat.all.6, aes(x = Site.Name,y=Abundance)) + 
  stat_summary(fun.y=mean, geom="bar",fill="white",colour="black") +
  stat_summary(fun.ymin = se.min, fun.ymax = se.max, geom = "errorbar", width = 0.1) +
  geom_hline(aes(yintercept=0))+
  ggtitle ("Trip 6 (November)")+
...
bar.location.legal.6

# plot of mean length ----
dat.length.6$Site
dat.length.6$Site<-factor(dat.length.6$Site, levels = c("Irwin Reef", "Whites Lump", "Jim Bailey","South Rig", "Little Horseshoe","Cliff Head OUT2","Cliff Head IN1", "Cliff Head IN2", "Golden Ridge"))
dat.length.6$Site

bar.length.location.6<-ggplot(subset(dat.length.6, Site %in% c("Irwin Reef","Whites Lump","Jim Bailey","South Rig","Little Horseshoe", "Cliff Head OUT2","Cliff Head IN1","Cliff Head IN2", "Golden Ridge")), aes(x = Site, y=Carapace.length)) + 
...
dat.map.sublegal<-dat.pot.5%>%
  dplyr::select(Trap.ID,Pot.Number,Day,Site.Name,Date.Recovered,Longitude.x,Latitude.y)%>%
  full_join(dat.sublegal.5)%>%
  filter(!Legal=="All")%>%
  rename(lon=Longitude.x,lat=Latitude.y)%>%
  mutate(lon=as.numeric(lon))%>%
  mutate(lat=as.numeric(lat))#%>%
...
## Using google theme ----
gg.map<-ggmap(get_map(location=center, source = "google", maptype= "satellite", zoom = 11))
