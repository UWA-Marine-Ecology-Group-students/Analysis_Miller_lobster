...
pred.vars.fact=c("Location","Sex")
# "Site"

pred.vars.cont=c("Carapace.length") #,"bearing"
# Removed correlated
# "Date",
...
for(i in 1:length(resp.vars)){
  ...
  Model1=gam(response~s(Carapace.length,k=3,bs='cr')+Location+s(Ldys,k=3,bs='re'),family=tw(),  data=use.dat)
  # gam.check(Model1)
  # summary(Model1)
  # plot.gam(Model1)
  # ?generate.model.set()
  model.set=generate.model.set(use.dat=use.dat,
                               ...
                               cov.cutoff = 0.23,
                               k=3,
                               null.terms="+s(Ldys,k=3,bs='re')") #s(Site,bs='re')+s(Date,bs='re')
