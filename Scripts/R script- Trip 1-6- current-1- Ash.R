...
dat.length<-gs_title("Lobsters_data_2018_Reds_20180511")%>% # For Brooke to use GoogleSheets
...
dat.pot<-gs_title("Lobsters_data_2018_Reds_20180511")%>% # For Brooke to use GoogleSheets
  gs_read_csv(ws = "Pot.var")%>%
  mutate(day.trap=paste(Day,Pot.Number,sep="."))%>%
  mutate(Site.Name=str_replace_all(.$Site.Name,c( "SM"="Seven Mile", "DM"="Davids Marks",  "RM"="Rivermouth", "IR"="Irwin Reef", "LR"="Long Reef", "SD"="South Dummy", "LH"="Little Horseshoe", "CHin1_"="Cliff Head IN1","CHin2_"="Cliff Head IN2","CHout1_" = "Cliff Head OUT1","CHout2_" = "Cliff Head OUT2", "JB"="Jim Bailey", "GR"="Golden Ridge", "SR"="South Rig", "WL"="Whites Lump")))%>% 
...
# Make factors----
dat.factors<-dat.pot%>%
  dplyr::select(Trip, Trap.ID,Pot.Number,Day,Site.Name,Date.Recovered)%>% 
  mutate(Site.Name=str_replace_all(.$Site.Name,c( "SM"="Seven Mile", "DM"="Davids Marks",  "RM"="Rivermouth", "IR"="Irwin Reef", "LR"="Long Reef", "SD"="South Dummy", "LH"="Little Horseshoe", "CHin1_"="Cliff Head IN1","CHin2_"="Cliff Head IN2","CHout1_" = "Cliff Head OUT1","CHout2_" = "Cliff Head OUT2", "JB"="Jim Bailey", "GR"="Golden Ridge", "SR"="South Rig", "WL"="Whites Lump")))%>% 
  distinct()%>%
  glimpse() 

unique(dat.factors$Trap.ID) # 372 pots

dat.factors$Site.Name
...
dat.sublegal<-dat.length%>%
  ...
  group_by(Trip, Trap.ID,Date.Recovered)%>%
  summarize(Abundance=n())%>%
  right_join(dat.factors,by = c("Trap.ID", "Date.Recovered", "Trip"))%>%
...
dat.legal<-dat.length%>%
  left_join(dat.factors)%>%
  filter(Carapace.length>76)%>%
  group_by(Trap.ID,Date.Recovered, Trip)%>%
  summarize(Abundance=n())%>%
  right_join(dat.factors,by = c("Trap.ID", "Date.Recovered", "Trip"))%>%
  replace_na(list(Abundance = 0))%>%
  mutate(Legal="Legal")%>%
...
dat.all<- bind_rows(dat.sublegal,dat.legal)

dat.all$Site.Name

# Check--
unique(dat.all$Legal)
...
dat.all$Site.Name<-factor(dat.all$Site.Name, levels = c( "SM"="Seven Mile", "RM"="Rivermouth", "IR"="Irwin Reef", "LR"="Long Reef", "WL"="Whites Lump",  "JB"="Jim Bailey",  "SR"="South Rig", "LH"="Little Horseshoe","SD"="South Dummy","CHout1_" = "Cliff Head OUT1","CHout2_" = "Cliff Head OUT2","DM"="Davids Marks", "CHin1_"="Cliff Head IN1","CHin2_"="Cliff Head IN2", "GR"="Golden Ridge"))
dat.all$Site.Name
...
#Abundance plot #### Not working for some reason
...
dat.legal$Site.Name<-factor(dat.legal$Site.Name, levels = c( "SM"="Seven Mile", "RM"="Rivermouth", "IR"="Irwin Reef", "LR"="Long Reef", "WL"="Whites Lump",  "JB"="Jim Bailey",  "SR"="South Rig", "LH"="Little Horseshoe","SD"="South Dummy","CHout1_" = "Cliff Head OUT1","CHout2_" = "Cliff Head OUT2","DM"="Davids Marks", "CHin1_"="Cliff Head IN1","CHin2_"="Cliff Head IN2", "GR"="Golden Ridge"))
glimpse(dat.legal)
dat.legal$Site.Name

# Rename sites

bar.location.legal<-ggplot(data = dat.all, aes(x = Site.Name,y=Abundance)) + 
  stat_summary(fun.y=mean, geom="bar",fill="white",colour="black") +
  stat_summary(fun.ymin = se.min, fun.ymax = se.max, geom = "errorbar", width = 0.1) +
  geom_hline(aes(yintercept=0))+
  ggtitle ("Trips 1-5")+
...
ggplot(data = dat.all, aes(x = Trip, y = Abundance, fill= Site.Name)) +
  stat_summary(fun.y=mean, geom="bar", stat="indentity") +
  stat_summary(fun.ymin = se.min, fun.ymax = se.max, geom = "errorbar", width = 0.1) +
  geom_hline(aes(yintercept=0))
...
# plot of mean length ----
dat.length$Site<-factor(dat.length$Site, levels = c( "SM"="Seven Mile", "RM"="Rivermouth", "IR"="Irwin Reef", "LR"="Long Reef", "WL"="Whites Lump",  "JB"="Jim Bailey",  "SR"="South Rig", "LH"="Little Horseshoe","SD"="South Dummy","CHout1_" = "Cliff Head OUT1","CHout2_" = "Cliff Head OUT2","DM"="Davids Marks", "CHin1_"="Cliff Head IN1","CHin2_"="Cliff Head IN2", "GR"="Golden Ridge"))

dat.length$Site
head(dat.length$Site)
...
library(data.table)
dat.legal[, .(Mean= rowMeans.(.SD)), by= Site.Name]

library(dplyr)
dat.legal%>%
  transmute(Site.Name, Mean=colMeans(select(.,-Site.Name)))

df<- data.frame(sub)

av.legal<- colMeans(dat.legal) 
dat.legal
